...
<script type="text/javascript">
	...
	function confirm(){
		alert("添加物品");
		window.location.href="${pageContext.request.contextPath }/iteminfo/seltItem.do";
	}
	function getaddjson() {
		$.ajax({
		...
		<div style="width: 90%;" align="center">
			<div >
				...
				<div id="popuporder" class="popup" style="padding: 20px">
					...
					<h3>新增采购订单</h3>
					<form class="form-inline" action="purchase/fileUpload.do" method="post" enctype="multipart/form-data">
						<fieldset>
							<table>
								<tr align="center">
									<td><label>采购订单号 </label> <input type="text" 
									...
									<td><label>申请人</label> <input type="text"
										...
										disabled="disabled" name="loginName"/></td>
								...
								<tr><td><label>供应商名称:</label><select name="chineseName" id="supplierName" onchange="selSupp();"
									class="form-control select-lg">
										<option></option>
										<option>--选择供应商名称--</option>
										<c:forEach items="${suppliers }" var="sups">
							...
							<tr>
								...
								<td><label>最后处理时间</label> <input type="text" name="commitTime"
										class="form-control select-lg" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/></td>
								</tr>
							</table>
							<div align="center">
						...
						<tbody>
							<c:forEach items="${gList }" var="group" varStatus="i">
								<tr ${(i.index+1)%2==0?"class='error'":"class='success'"}>
									<td></td>
									<td><input type="hidden" id="iid" value="${group.purchaseOrderID}"></td>
									<td>${i.index+1}</td>
